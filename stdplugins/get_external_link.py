...
def get_link(bot, update):
    ...
    if update.reply_to_message is not None:
        ...
        if str(update.from_user.id) in Config.G_DRIVE_AUTH_DRQ:
            ...
            url = "https://srv-file1.gofile.io/ajax/upload.php"
            max_days = 5
            command_to_exec = [
                "curl",
            ...
            try:
                ...
                t_response_arry = t_response.decode("UTF-8").split(";")[1].strip()
                
                #shorten_api_url = "http://ouo.io/api/{}?s={}".format(Config.OUO_IO_API_KEY, t_response_arry)
                #adfulurl = requests.get(shorten_api_url).text
        bot.edit_message_text(
            chat_id=update.chat.id,
            text=Translation.AFTER_GET_DL_LINK.format("https://gofile.io/?c="+t_response_arry, max_days),
            parse_mode=pyrogram.ParseMode.HTML,
            message_id=a.message_id,
            disable_web_page_preview=True
        )
